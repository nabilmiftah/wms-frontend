...
<template>
  <AuthLayout>
    ...
    <form @submit.prevent class="space-y-5">
      ...
      <button class="w-full border border-slate-200 py-3 rounded-xl flex items-center justify-center gap-3 font-bold text-slate-700">
        <img src="/icon-google.png" class="w-5" /> Google
      </button>

      <p class="text-center text-xs mt-6 text-slate-500">
        Don't have an account? <router-link to="/signup" class="text-blue-600 font-bold underline">register here</router-link>
      </p>
    </form>
  </AuthLayout>
</template>