  ...
  <MainLayout>
    <div class="p-8 bg-[#F8F9FB] min-h-screen space-y-8">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-slate-500 text-sm">
            Monitor aktivitas warehouse dan inventory secara real-time.
          </p>
        </div>
        <div class="flex gap-3">
          <BaseButton
            class="bg-[#2563EB] text-white px-5 py-2.5 rounded-xl shadow-lg shadow-blue-100 flex items-center gap-2 font-medium"
          >
            <i class="fas fa-plus-circle"></i> Add Asset
          </BaseButton>
          <BaseButton
            class="bg-white border border-slate-200 text-slate-600 px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-sm font-medium"
          >
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        >
          <div class="flex justify-between items-start mb-4">
            ...
            <span
              :class="[
                'text-[10px] font-bold px-2 py-1 rounded-lg bg-slate-50 border border-slate-100',
                item.trendColor,
              ]"
            >
              {{ item.trend }}
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div
          class="lg:col-span-2 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm"
        >
          <div class="flex justify-between items-center mb-6">
            <h3 class="font-bold text-slate-800">
              Inbound / Outbound Asset Chart
            </h3>
            <div class="flex bg-slate-50 p-1 rounded-lg">
              <button
                class="px-4 py-1 text-xs font-bold bg-white text-blue-600 rounded-md shadow-sm"
              >
                Weekly
              </button>
              <button
                class="px-4 py-1 text-xs font-bold text-slate-400 hover:text-slate-600 transition"
              >
                Monthly
              </button>
            </div>
        ...
        <div
          class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm flex flex-col"
        >
                ...
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    <p class="text-xs font-bold text-slate-800 leading-none">
                      {{ donutOptions.labels[i] }}
                    </p>
                    <p class="text-[9px] text-slate-400 mt-1">
                      {{ i === 0 ? "45%" : i === 1 ? "35%" : "20%" }} dari total
                    </p>
                  </div>
                </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
        ...
        >
          <div class="flex justify-between items-center mb-4">
            ...
            <button class="text-blue-600 text-[10px] font-bold hover:underline">
              Lihat semua
            </button>
          </div>
          <BaseTable
